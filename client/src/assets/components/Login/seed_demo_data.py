...
# Drop your own photos in campuslife/fixtures/demo_media/ using these exact
# filenames. Each lodge gets its own exterior photo; all interiors share
# one photo. If a file is missing, that record is still created — it just
# won't have a picture attached, and a warning prints.
...
# Fixed date so seeding is deterministic across runs/machines.
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # --- Lodges — each with its own lodge photo + interior photo ---
        lodge_specs = [
            {
        ...
        # --- Rooms (Room.room is the FK to Agent — confusingly named, see WIRING_GUIDE note) ---
